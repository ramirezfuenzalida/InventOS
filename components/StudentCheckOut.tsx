              ...
              <div className="space-y-4">
                {searchResults.map((group) => (
                  <button
                    ...
                    className="w-full bg-slate-900/40 border border-white/5 p-8 rounded-[2.5rem] flex items-center justify-between hover:bg-slate-800/60 hover:border-indigo-500/30 transition-all group/card text-left"
                  >
                    <div className="flex items-center gap-8 min-w-0 flex-1">
                      <div className="w-16 h-16 rounded-[1.5rem] flex items-center justify-center bg-indigo-500/10 text-indigo-400 flex-shrink-0">
                        <User className="w-8 h-8" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="text-white font-black uppercase italic text-xl leading-none mb-2 truncate">
                          {group.studentName}
                        </p>
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0 ml-4">
                      {group.instruments.length > 1 && (
                        <span className="text-[9px] font-black bg-indigo-600 text-white px-3 py-1.5 rounded-full">
                          {group.instruments.length} INST.
                        </span>
                      )}
                      <ArrowRight className="w-6 h-6 text-slate-700 group-hover/card:text-white transition-all" />
                    </div>
                  </button>
                ))}
                ...
                {searchTerm && searchResults.length === 0 && (
                  <div className="py-20 text-center border-2 border-dashed border-white/5 rounded-[3rem]">
                    ...
                    <p className="text-slate-700 font-black uppercase italic tracking-[0.2em] mb-2">
            ...
            <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
              <div className="flex items-center gap-4">
                <button
                  ...
                  className="p-4 bg-slate-950 rounded-[1.5rem] text-slate-500 hover:text-white transition-all shadow-xl"
                >
                  <X className="w-6 h-6" />
                </button>
                <div>
                  <p className="text-indigo-400 font-black text-[10px] uppercase tracking-[0.4em]">ESTUDIANTE SELECCIONADO</p>
                  <h3 className="text-2xl md:text-3xl font-black text-white italic uppercase tracking-tighter">
                    {selectedStudent.studentName}
                  </h3>
                  <p className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em]">{selectedStudent.course}</p>
                </div>
              </div>

              <p className="text-slate-500 text-xs font-black uppercase tracking-[0.3em] text-center">
              ...
              <div className="space-y-4">
                {selectedStudent.instruments
                  ...
                  <button
                    ...
                    className="w-full bg-slate-900/40 border border-white/5 p-8 rounded-[2.5rem] flex items-center justify-between hover:bg-slate-800/60 hover:border-indigo-500/30 transition-all group/inst text-left"
                  >
                    <div className="flex items-center gap-8">
                      <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center ${isItemLoaned(inst) ? 'bg-amber-500/10 text-amber-500' : 'bg-indigo-500/10 text-indigo-400'}`}>
                        <Music className="w-8 h-8" />
                      </div>
                      <div>
                        <p className="text-white font-black uppercase italic text-xl leading-none mb-2">{inst.Instrumento}</p>
                        <div className="flex flex-wrap gap-3">
                          <span className="text-[10px] font-black text-indigo-500 uppercase tracking-widest">{inst.Marca}</span>
                          <span className="text-[10px] font-bold text-slate-600">|</span>
                          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{inst.Modelo}</span>
                          {inst.Serie && (
                            <>
                              <span className="text-[10px] font-bold text-slate-600">|</span>
                              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">S/N: {inst.Serie}</span>
                            </>
                          )}
                        </div>
                        <p className={`text-[10px] font-black uppercase tracking-[0.2em] mt-2 ${isItemLoaned(inst) ? 'text-amber-500' : 'text-emerald-500'}`}>
